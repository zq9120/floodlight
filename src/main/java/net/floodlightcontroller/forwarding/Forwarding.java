...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener, ILinkDiscoveryListener,
	...
	/**
	 * L3 routing block -- it supports routing across subnet by using "virtual
	 * gateway". It also keeps L2 forwarding as Floodlight original does
	 *
	 * @param eth             Ethernet Frame that associated with this OFPacketIn
	...
	/**
	 * This function retrieves a patch from source device to destination device and
	 * then install flows(L3/L2) over the path. In L3 case, virtual router caches
	 * the packet-in while it sends the ARP message to learn destination device
	 *
	 * @param sw
	 * @param pi
	 * @param decision
	...
	/**
	 * Virtual gateway insert flows on switch to rewrite source MAC to gateway MAC,
	 * also rewrite destination MAC to destination host.
	 *
	...
	/**
	 * L3 forwarding block -- it supports L2 forwarding from a source to a
	 * destination. No L3 functionality involved.
	 *
	...
	/**
	 * This function retrieves a patch from source device to destination device and
	 * then install L2 flows over the path.
	 *
	...
	/**
	 * Generate arp reply packet so virtual gateway can use it to response the
	 * cross-subnet ARP request sent from host
	 *
	 * @param cntx       The FloodlightContext associated with this OFPacketIn
	...
	public IPacket gatewayArpReply(FloodlightContext cntx, MacAddress gatewayMac) {
		...
		return new Ethernet().setSourceMACAddress(gatewayMac).setDestinationMACAddress(eth.getSourceMACAddress())
				.setEtherType(EthType.ARP).setVlanID(eth.getVlanID()).setPriorityCode(eth.getPriorityCode())
	...
	/**
	 * Push arp packet to the network when virtual gateway needs to response arp
	 * request from host
	 *
	...
	public void pushArpReply(IPacket packet, IOFSwitch sw, OFBufferId bufferId, OFPort inPort, OFPort outPort) {
		if (log.isTraceEnabled()) {
			log.trace("Push ar PacketOut srcSwitch={} inPort={} outPort={}", new Object[] { sw, inPort, outPort });
		}
	...
	/**
	 * Find corresponding virtual gateway interface based on the destination IP
	 * address
	 *
	...
	/**
	 * Query device based on packet destination IP address, rather than based on MAC
	 * address This is because in L3 case, the MAC address is Virtual Router MAC,
	 * which is not exist in device map
	 *
	 * @param dstIP packet destination IP
	 * @return
	 */
	 ...
	 * messages.
	 *
	 * @param maskedDescriptors A sequence of masked cookies describing
	 *                          IRoutingDecision descriptors
	 ...
	 *
	 * @param descriptors The descriptors and masks describing which flows to
	 *                    delete.
	 */
	...
	protected void doFloodMigrate(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
		if (FlowGen.flowGenStatus) {
			...
			List<OFAction> actions = new ArrayList<>();
			Set<OFPort> broadcastPorts = this.topologyService.getSwitchBroadcastPorts(swRoot.getId());
			...
			for (OFPort p : broadcastPorts) {
				if (p.equals(inPort))
					continue;
				actions.add(swRoot.getOFFactory().actions().output(p, Integer.MAX_VALUE));
			}
			pob.setActions(actions);
			...
			if (log.isTraceEnabled()) {
				log.info("Writing flood PacketOut switch={} packet-in={} packet-out={}",
						new Object[] { swRoot, pi, pob.build() });
			}
	...
	/**
	 * Virtual gateway creates OFPacketOut with ARP request message and flood it to
	 * network, for learning destination host purpose
	 *
	...
	class DeviceListenerImpl implements IDeviceListener {
		...
		public void deviceIPV4AddrChanged(IDevice device) {
			// If the device updated IP associated with the packet in the cache, release
			// that packet
	...
	private Optional<Ethernet> findPacketInCache(Map<OFPacketIn, Ethernet> l3cache, IPv4Address ip) {
		return l3cache.values().stream()
				.filter(packet -> ((IPv4) packet.getPayload()).getDestinationAddress().equals(ip)).findAny();
	}
	...
	private void pushL3Packet(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx, IDevice dstDevice) {
		...
		if (pi.getBufferId().equals(OFBufferId.NO_BUFFER)) {
			...
			log.error("BufferId configured on switch. " + "Cannot send packetOut. " + "srcSwitch={} inPort={}",
